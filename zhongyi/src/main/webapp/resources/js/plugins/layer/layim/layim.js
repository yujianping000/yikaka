...
var config = {
    ...
    right: -232,
    api: {
        friend: framework.base + '/admin/common/friends.json', //好友列表接口
        chatlog: 'js/plugins/layer/layim/data/chatlog.json', //聊天记录接口
        sendurl: '' //发送消息接口
    },
    user: { //当前用户信息
        name: '我',
        face: '../..//resources/img/logo-login.png'
    },
...
xxim.popchat = function(param){
    ...
    log.success = function(layero){
        ...
        log.chatlist.html('<li data-id="'+ param.id +'" type="'+ param.type +'"  data-account="'+param.account+'" id="layim_user'+ param.type + param.id +'"><span>'+ param.name +'</span><em>×</em></li>')
    ...
    log.html = '<div class="layim_chatbox" id="layim_chatbox">'
            ...
            +'        <ul class="layim_chatview layim_chatthis" data-account="'+param.account+'"  id="layim_area'+ param.type + param.id +'"></ul>'
            +'    </div>'
    ...
    if(config.chatings < 1){
        ...
        log.chatmore.find('ul').append('<li data-id="'+ param.id +'" data-account="'+param.account+'" type="'+ param.type +'" id="layim_user'+ param.type + param.id +'" class="layim_chatnow"><span>'+ param.name +'</span><em>×</em></li>');
        
        log.chatarea.find('.layim_chatview').removeClass('layim_chatthis');
        log.chatarea.append('<ul class="layim_chatview layim_chatthis"  data-account="'+param.account+'" id="layim_area'+ param.type + param.id +'"></ul>');
...
//弹出聊天窗2
xxim.popchatbox2 = function(othis,content){
    var node = xxim.node, dataId = othis.attr('data-id'), param = {
        id: dataId, //用户ID
        account:  othis.attr('data-account'), //用户帐号
...
xxim.transmit = function(){
    ...
    log.send = function(){
        ...
        if(data.content.replace(/\s/g, '') === ''){
            ...
            //测试
            //Strophex.sendMessage('admin@119.29.0.81',data.content );
            //正式
            Strophex.sendMessage(data.account+"@" + Strophex.domain,data.content);
        }
...
xxim.event = function(){
    ...
    node.xximSearch.keyup(function(){
        ...
        if(val !== ''){
            ...
            var length = $(".xxim_childnode").find(".xxim_onename:contains('"+val+"')");
            if(length == 0){
            	node.list.eq(3).html('<li class="xxim_errormsg">没有符合条件的结果</li>');
        	}else{
        		node.searchMian.html('');
        		var childs = $(".xxim_childnode").find(".xxim_onename:contains('"+val+"')");
        		childs.each(function(){
        			node.searchMian.append($(this).parent().clone());
        		});
        	}
        } else {
                ...
                if(index !== 2){
                    for(; i < myflen; i++){
                        ...
                        for(var j = 0; j < item.length; j++){
                            str += '<li data-id="'+ item[j].id +'" data-account="'+item[j].account+'" class="xxim_childnode" type="'+ (index === 0 ? 'one' : 'group') +'"><img src="'+ item[j].face +'" class="xxim_oneface"><span class="xxim_onename">'+ item[j].name +'</span></li>';
                        }
                        str += '</ul></li>';
                    }
                } else {
...
xxim.recive = function(account,content){
	if(content == null || content==""){return;}
	$(".xxim_childnode").each(function(){
		if($(this).attr('data-account') == account){
			 playSound();
			 xxim.popchatbox2($(this),content);
		}
	});
};
...
xxim.view = (function(){
    var xximNode = xxim.layimNode = $('<div id="xximmm" class="xxim_main">'
            ...
            +'  <ul class="xxim_chatlist xxim_list xxim_searchmain" id="xxim_searchmain"></ul>'
            +'</div>'
