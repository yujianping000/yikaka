                            ...
                            <table id="listTable" class="table table-striped table-bordered table-hover dataTables-example dataTable">
                                ...
                                <tbody>
                                	[#list paymentPlugins as paymentPlugin]
										<tr class="gradeX [#if (paymentPlugin_index+1) % 2 != 0]odd[#else]even[/#if]" role="row">
											<td>
											...
											<td>
												[#if paymentPlugin.isInstalled]
													[#if paymentPlugin.settingUrl??]
														<a href="${paymentPlugin.settingUrl}" class="btn btn-white btn-xs"> <i class="fa fa-gears"></i>&nbsp;${message("admin.paymentPlugin.setting")}</a>
													[/#if]
													[#if paymentPlugin.uninstallUrl??]
														<a href="javascript:;" data-href="${paymentPlugin.uninstallUrl}" class="btn btn-danger btn-xs uninstall"><i class="fa fa-wrench"></i>&nbsp;${message("admin.paymentPlugin.uninstall")}</a>
													[/#if]
												[#else]
													[#if paymentPlugin.installUrl??]
														<a href="javascript:;" data-href="${paymentPlugin.installUrl}" class="btn btn-success btn-xs install"><i class="fa fa-puzzle-piece"></i>&nbsp;${message("admin.paymentPlugin.install")}</a>
													[/#if]
												[/#if]
											</td>
										</tr>
