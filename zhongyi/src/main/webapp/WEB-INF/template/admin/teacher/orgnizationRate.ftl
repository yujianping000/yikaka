        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                	[#if orgId??]
                		<form id="listForm" action="${base}/admin/teacher/orgnizationRate/${teacher.id}/${paramter.orgId}.jhtml" method="get">
                	[#else]
                		<form id="listForm" action="${base}/admin/teacher/orgnizationRate/${teacher.id}.jhtml" method="get">
                	[/#if]
    ...
    <script>
        $(document).ready(function () {
			...
			$(".startOrgnizationRate").click(function(){
		    	var $this = $(this);
				 		$.ajax({
							url: "${base}/admin/teacher/startOrgnizationRate.jhtml",
							type: "POST",
			...
			$(".forbidOrgnizationRate").click(function(){
				...
				var $this = $(this);
				 bootbox.confirm("确定禁用?", function(result) {
				 	if(result){
				 		$.ajax({
							url: "${base}/admin/teacher/forbidOrgnizationRate.jhtml",
							type: "POST",
							data: {id:$this.attr('data-id')},
							dataType: "json",
							cache: false,
