...
@Component("UpdateCommentOrderJob")
@Lazy(false)
public class UpdateCommentOrderJob{
 	@Resource
	...
	@Scheduled(cron = "*/50 * * * * ?")  //每30钟执行一次，培训完成的订单修改订单状态为未评价状态
		public void build() {
	    Long markTime= Long.parseLong(StringUtil.date2String3(new Date()));

	    Map map=new HashMap();
	    map.put("status", 2);
	    map.put("classtimes", 0);
	  ...
	  log.info("凌晨3点执行，培训完成的订单修改订单状态为未评价状态,成功!");
	}
} 
