 ...
 * @version 2.0
 */
@Controller("adminActivityOrderController")
@RequestMapping({ "/admin/activity_order" })
public class ActivityOrderController extends GenericController {
	...
	public String add(ModelMap model) {
		return "/admin/activity_order/add";
	}
	...
	public String edit(@PathVariable Long id, ModelMap model) {
		...
		return "/admin/activity_order/edit";
	}
	...
	public String view(@PathVariable Long id, ModelMap model) {
		Order order = orderService.find(id);
		model.addAttribute("order", order);
		return "/admin/activity_order/view";
	}
	...
	public String list(Pageable pageable,HttpServletRequest request, ModelMap model) {
		...
		return "/admin/activity_order/list";
	}
	...
	private Map RequestMap(HttpServletRequest request) {
		...
		String filter_targetName = request.getParameter("filter_targetName");
		...
		if(!StringUtil.isEmpty(filter_targetName)) {
			paramter.put("filter_targetName", StringUtil.filterSQL(filter_targetName));
		}
	 ...
	 * 导出活动订单列表
	 */
	@RequestMapping(value="activityExport",method={RequestMethod.GET,RequestMethod.POST})
	public ModelAndView activityExport(ModelMap model,HttpServletRequest request,HttpServletResponse response) {
		Map paramter = RequestMap(request);
		
		 List<Order> activityOrderList = orderService.findList(paramter);
		
		String fileName = "活动订单列表" + ".xls";
		
 		return new ModelAndView(new ExcelView(fileName, fileName,
 				new String[] {"bn","phone","targetName","totalFee","createdTimeString","payTimeString","phone","statusString"}, 
 				new String[] {"订单编号","下单账号","活动名称","订单金额（元）","下单时间","支付时间","联系电话","状态"},
 				new Integer[] {5000,5000,5000,5000,5000,5000,5000,5000}, 
 				null,activityOrderList, null), model);
